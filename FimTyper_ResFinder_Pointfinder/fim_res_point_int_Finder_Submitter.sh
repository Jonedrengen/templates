...
# Created by Edward Sung (edward.sung@gwu.edu) on 2/10/25
# Version v1
...
$scriptLocation/Slurm_Array_SampleListReady.sh $Data_Folder_Samplelist_input
...
do
   ...
   echo "sbatch --array=${array_start}-${array_end}%${Slurm_CalcRunParallel} -J $jobname $scriptLocation/fim_res_point_int_Finder_Runner.sh $Data_Folder_input ${samplelist_filename}_SLURM-ARRAY-READY.txt $index_set ${Job_Name_input}_output"
   sbatch --array=$array_start-$array_end%$Slurm_CalcRunParallel -J $jobname $scriptLocation/fim_res_point_int_Finder_Runner.sh $Data_Folder_input ${samplelist_filename}_SLURM-ARRAY-READY.txt $index_set ${Job_Name_input}_output
done

# Compile the results data and Clean-up file system script
sbatch --dependency=singleton -J $jobname $scriptLocation/fim_res_point_int_Finder_Compiler.sh ${Job_Name_input}_output $jobname
